...
TopDownGame.Pathfinding = function(game, parent){
    Phaser.Plugin.call(this, game, parent);
    this.easy_star = new EasyStar.js();
};

TopDownGame.Pathfinding.prototype = Object.create(Phaser.Plugin.prototype);
TopDownGame.Pathfinding.prototype.constructor = TopDownGame.Pathfinding;

TopDownGame.Pathfinding.prototype.init = function(world_grid, acceptable_tiles, tile_dimensions){
    var grid_row, grid_column, grid_indices;
    this.grid_dimensions = {row: world_grid.length, column: world_grid[0].length};

    grid_indices = [];
    for(grid_row = 0; grid_row < world_grid.length; grid_row++){
        grid_indices[grid_row] = [];
        for(grid_column = 0; grid_column < world_grid[grid_row].length; grid_column++){
...
TopDownGame.Pathfinding.prototype.find_path = function(origin, target, callback, context){
...
TopDownGame.Pathfinding.prototype.call_callback_function = function(callback, context, path){
    var path_positions;
    path_positions = [];
    if(path !== null){
...
TopDownGame.Pathfinding.prototype.outside_grid = function(coord){
    return coord.row<0 || coord.row>this.grid_dimensions.row-1 || coord.column<0 || coord.column>this.grid_dimensions.column-1;
}

TopDownGame.Pathfinding.prototype.get_coord_from_point = function(point){
    var row, column;
...
TopDownGame.Pathfinding.prototype.get_point_from_coord = function(coord){
    var x, y;
...
TopDownGame.Pathfinding.prototype.changeMap = function(world_grid){
